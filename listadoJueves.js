...
var nombreArchivo = CodigoEntrenador + "_Prospectos_Registro_Seminario_Jueves_" + (day + "/" + (month + 1) + "/" + year);
...
function getData() {
    var prospectosRef = firebase.database().ref('prospecto/jueves');
...
function actualizarProspectosNuevos() {
    for (i = 0; i < ProspectosNuevos.length; i++) {
        var updateRef = firebase.database().ref('prospecto/jueves/' + ProspectosNuevos[i].key);
        updateRef.update({
            visto: 1
        });
    }
